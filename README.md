...
The library needs that same agreed value itself, and will not invent one: before calling `Start()` on a keygen, signing or re-sharing party you **must** call `Parameters.SetSessionNonce` with a value all participants in that run have agreed on. It is mixed into the SSID that binds every zero-knowledge proof in the run, so a proof produced in one run is not accepted in another, and round 1 returns an error if it has not been set (the two paths that derive no session ID of their own — EdDSA re-sharing, and the new committee's first round in re-sharing — do not check it; set it on every party anyway). Note that `Parameters` belongs to a party rather than to a single execution: if you keep one around across runs, set a fresh nonce for each of them, or every run will share an SSID.
