...
import (
	...
	resharing "github.com/bnb-chain/tss-lib/v4/eddsa/resharing"
	"github.com/bnb-chain/tss-lib/v4/test"
	"github.com/bnb-chain/tss-lib/v4/tss"
)

// TestResharingRejectsSlotZeroPubKeySwap is the EdDSA counterpart of the ECDSA
// regression test for SRC-2026-1155 ("Resharing Slot-0 Public-Key Swap"). The
// new committee's round-1 continuity check must reject an old-committee party
// that advertises an aggregate public key differing from the other old parties.
//
...
// "every old party must advertise the same key" comparison never fired and a
// malicious old slot-0 party could swap the reshared public key undetected.
// Against the buggy code this test times out (no error is ever raised).
func TestResharingRejectsSlotZeroPubKeySwap(t *testing.T) {
	...
	select {
	case err := <-errCh:
		assert.Contains(t, err.Error(), "eddsa pub key did not match",
